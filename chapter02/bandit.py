...
class ArmedBanditTestbedEnv(gym.Env):
    ...
    def step(self, arm):
        ...
        return 0, reward, False, False, {'arm_star': np.argmax(self.q_star)}
    ...
    def render(self, mode='human'):
        ...
        for arm in range(self.action_space.n):
            bandit_samples += [np.random.normal(loc=self.q_star[arm], scale=1.0, size=1000)]
...
def run_episode(bandit, gambler, seed=None):
    ...
    while not done and not truncated:
        arm = gambler.arm()
        state, reward, done, truncated, info = bandit.step(arm)
