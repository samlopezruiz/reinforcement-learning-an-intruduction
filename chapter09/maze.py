...
class ShortcutMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        return position, reward, done, done, {}
...
class MultiDiscreteObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        assert isinstance(env.observation_space,
                          MultiDiscrete), "The environment must have a MultiDiscrete observation space."

        self.flattened_space_size = np.prod([n for n in env.observation_space.nvec])

        self.observation_space = Discrete(self.flattened_space_size)

    def observation(self, observation):
        # Convert the multidimensional index to a single-dimensional index
        return np.ravel_multi_index(observation, self.env.observation_space.nvec)
